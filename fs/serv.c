...
int serve_read(envid_t envid, union Fsipc *ipc)
{
	...
	if ((fret = openfile_lookup(envid, req->req_fileid, &of)) != 0) {
		return fret;
	}
	rsize = file_read(of->o_file, ret->ret_buf, req->req_n, of->o_fd->fd_offset);

	of->o_fd->fd_offset += rsize;
...
int serve_write(envid_t envid, struct Fsreq_write *req)
{
	...
	rsize = file_write(of->o_file, req->req_buf, req->req_n, of->o_fd->fd_offset);
