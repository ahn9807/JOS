...
boot_alloc(uint32_t n)
{
	...
	if (n==0) return (void *)nextfree;
		...
		uint64_t start = (uint64_t) nextfree;
		uint64_t new = (n/PGSIZE) + 1;
		result = (char *) (start + new);
		return result;
	}*/
	result = nextfree;
	if (n == 0)
	...
	extern char edata[];
	if ((uintptr_t)result > (uintptr_t)end + npages * PGSIZE)
	{
		panic("oom");
	}
	memset(result, 0, ROUNDUP(n, PGSIZE));
...
mem_init_mp(void)
{
	...
	for(int i=0;i<NCPU;i++){
		...
		boot_map_region(boot_pml4e, (uintptr_t) kstacktop_i - KSTKSIZE, (size_t) KSTKSIZE, PADDR(percpu_kstacks[i]), PTE_P | PTE_W);
	}
...
page_init(void)
{
	...
	for (i=0;i<npages;i++){
		...
		//Boot page table
		else if (i>=PADDR(BOOT_PAGE_TABLE_START)/PGSIZE && i<PADDR(BOOT_PAGE_TABLE_END)/PGSIZE){
			pages[i].pp_ref = 1;
			pages[i].pp_link = NULL;
		}

		//Boot alloc section
		else if(i>=EXTPHYSMEM/PGSIZE && i<(PADDR(boot_alloc(0))/PGSIZE + 1)){
			pages[i].pp_ref = 1;
			pages[i].pp_link = NULL;
		}
...
page_alloc(int alloc_flags)
{
	...
	/*struct PageInfo *new = &page_free_list[0];
...
pml4e_walk(pml4e_t *pml4e, const void *va, int create)
{
	...
	uint64_t *pml4e_entry = &(pml4e[PML4(va)]);
	
	if(!PTPRESENT(*pml4e_entry)){
		if(create){
			...
			if(new_page){
				...
				new_page->pp_ref++;
				is_page_allocated++;
				//if((uint64_t)va==0) cprintf("### pml4 new_page:%x, ref:%d\n",new_page,new_page->pp_ref);
				*pml4e_entry = page2pa(new_page) | PTE_P | PTE_U | PTE_W;
			}
			else{
				return NULL;
			}
...
pdpe_walk(pdpe_t *pdpe,const void *va,int create){
	...
	uint64_t *pdpe_entry = &(pdpe[PDPE(va)]);
	if(!PTPRESENT(*pdpe_entry)){
		if(create){
			new_page = page_alloc(ALLOC_ZERO);
			if(new_page){
				...
				new_page->pp_ref++;
				is_page_allocated++;
				//if((uint64_t)va==0) cprintf("### pdp new_page:%x, ref:%d\n",new_page,new_page->pp_ref);
				*pdpe_entry = page2pa(new_page) | PTE_P | PTE_U | PTE_W;
	...
	// physaddr_t __m_pa = PTE_ADDR(*pdpe_entry);
...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
	...
	if(!PTPRESENT(*pgdir_entry)){
		if(create){
			...
			if(new_page){
				...
				new_page->pp_ref++;
				is_page_allocated++;
				//if((uint64_t)va==0) cprintf("### pgdir new_page:%x, ref:%d\n",new_page,new_page->pp_ref);
	...
	// When __m_ppn >= npages, KADDR causes panic.
	/*
	physaddr_t __m_pa = PTE_ADDR(*pgdir_entry);
	uint32_t __m_ppn = PPN(__m_pa);
	if (__m_ppn >= npages) return NULL;
	*/
	pte = (pte_t *)KADDR(PTE_ADDR(*pgdir_entry));
	pte = &pte[PTX(va)];
...
boot_map_region(pml4e_t *pml4e, uintptr_t la, size_t size, physaddr_t pa, int perm)
{
	...
	for(addr=0;addr<size;addr += PGSIZE){
		pte = pml4e_walk(pml4e, (void *)(la + addr), 1);
		assert(pte != NULL);

		*pte = (pa + addr) | perm | PTE_P;
	}
}
...
page_insert(pml4e_t *pml4e, struct PageInfo *pp, void *va, int perm)
{
	...
	*pte = page2pa(pp) | perm | PTE_P;
	//cprintf("insert pte %x with perm %x\n", *pte, perm);
	return 0;
...
page_remove(pml4e_t *pml4e, void *va)
{
	// Fill this function in
	pte_t *pte_store;
	//cprintf("check va2pa: 0x%x\n",check_va2pa(pml4e, (uintptr_t)va));
