...
sys_exofork(void)
{
	...
	if(res == -E_NO_FREE_ENV){
		curenv->env_tf.tf_regs.reg_rax = -E_NO_FREE_ENV;
		return -E_NO_FREE_ENV;
	}
	if(res == -E_NO_MEM){
		curenv->env_tf.tf_regs.reg_rax = -E_NO_MEM;
		return -E_NO_MEM;
	}
	...
	curenv->env_tf.tf_regs.reg_rax = new_env->env_id;
	//cprintf("exofork success\n");
...
sys_page_alloc(envid_t envid, void *va, int perm)
{
	...
	if(res == -E_BAD_ENV){
		curenv->env_tf.tf_regs.reg_rax = res;
		return res;
	}
	if(((uint64_t) va) >= UTOP || ((uint64_t)va)%PGSIZE != 0 || (perm & PTE_SYSCALL) != perm){
		curenv->env_tf.tf_regs.reg_rax = -E_INVAL;
		return -E_INVAL;
	}
	...
	res = page_insert(target->env_pml4e, page, va, perm);
	...
	curenv->env_tf.tf_regs.reg_rax = 0;
	//cprintf("page alloc success with perm %x\n",perm);
	return 0;
	     ...
	     envid_t dstenvid, void *dstva, int perm)
{
	...
	int rs = envid2env(srcenvid, &srcenv, 1);
	int rd = envid2env(dstenvid, &dstenv, 1);
	if(rs!=0 || rd!=0){
		curenv->env_tf.tf_regs.reg_rax = -E_BAD_ENV;
		return -E_BAD_ENV;
	}
	pte_t *pte_store;
	struct PageInfo * page = page_lookup(srcenv->env_pml4e, srcva, &pte_store);
	...
	if((perm & PTE_W) && (!(((uint64_t)*pte_store) & PTE_W))){
		
		curenv->env_tf.tf_regs.reg_rax = -E_INVAL;
		return -E_INVAL;
	}
	
	if(page_insert(dstenv->env_pml4e, page, dstva, perm)!=0){
		curenv->env_tf.tf_regs.reg_rax = -E_NO_MEM;
		return -E_NO_MEM;
	}
	curenv->env_tf.tf_regs.reg_rax = 0;
...
sys_page_unmap(envid_t envid, void *va)
{
	...
	page_remove(target->env_pml4e, va);
	curenv->env_tf.tf_regs.reg_rax = 0;
	return 0;
