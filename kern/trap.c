...
trap_init_percpu(void)
{
	...
	int cpui = thiscpu->cpu_id;
	thiscpu->cpu_ts.ts_esp0 = KSTACKTOP - cpui * (KSTKSIZE + KSTKGAP);

	// Initialize the TSS slot of the gdt.
	//SETTSS((struct SystemSegdesc64 *)((gdt_pd>>16)+40),STS_T64A, (uint64_t) (&ts),sizeof(struct Taskstate), 0);
	SETTSS((struct SystemSegdesc64 *)&gdt[(GD_TSS0 >> 3) + cpui], STS_T64A, (uint64_t) (&thiscpu->cpu_ts), sizeof(struct Taskstate), 0);
...
trap_dispatch(struct Trapframe *tf)
{
	...
	if (tf->tf_trapno == T_PGFLT){
		...
		//cprintf("trap dispatch syscallno : %d\n",tf->tf_regs.reg_rax);
	...
	//cprintf("#### unexpected trap with trap no %x, SC %x ####\n",tf->tf_trapno,T_SYSCALL);
	print_trapframe(tf);
	if (tf->tf_cs == GD_KT)
